...
describe('password character pools', () => {
  it.each([
    ...
    const password = generatePassword({
      length: 32,
      options: { ...DEFAULT_OPTIONS, lowercase: false, uppercase: false, numbers: false, symbols: false, [selection]: true },
...
describe('secure random selection', () => {
  ...
  it('uses the supplied secure random source for every password character', () => {
    ...
    generatePassword({ length: 4, options: { ...DEFAULT_OPTIONS, uppercase: false, numbers: false, symbols: false }, getRandomValues: randomValues });
...
describe('password boundaries and entropy', () => {
  it('supports minimum and maximum lengths and batch sizes', () => {
    ...
    expect(generatePassword({ length: MIN_PASSWORD_LENGTH, getRandomValues: randomValues })).toHaveLength(MIN_PASSWORD_LENGTH);
    expect(generatePassword({ length: MAX_PASSWORD_LENGTH, getRandomValues: randomValues })).toHaveLength(MAX_PASSWORD_LENGTH);
    expect(generatePasswordBatch({ batchSize: MAX_BATCH_SIZE, getRandomValues: randomValues })).toHaveLength(MAX_BATCH_SIZE);
  });
  ...
  it('rejects invalid lengths, batch sizes, and an empty character selection', () => {
    ...
    expect(() => generatePassword({ options: { ...DEFAULT_OPTIONS, lowercase: false, uppercase: false, numbers: false, symbols: false } })).toThrow(/Select at least one/);
  });
