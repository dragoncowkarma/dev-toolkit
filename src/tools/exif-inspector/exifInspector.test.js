...
function buildGpsTiff() {
  ...
  u32(22, 0);
  u16(26, 4);
  const gpsEntries = [[1, 2, 2, 0x4e], [2, 5, 3, 80], [3, 2, 2, 0x57], [4, 5, 3, 104]];
  gpsEntries.forEach((entry, index) => {
    const offset = 28 + index * 12;
    u16(offset, entry[0]);
    u16(offset + 2, entry[1]);
    u32(offset + 4, entry[2]);
    u32(offset + 8, entry[3]);
  });
  u32(76, 0);
...
function makeJpeg(tiff) {
  ...
  return Uint8Array.from([
    0xff, 0xd8, 0xff, 0xe1, length >> 8, length & 0xff, 0x45, 0x78,
    0x69, 0x66, 0, 0, ...tiff, 0xff, 0xd9,
  ]);
}
...
describe('parseExif', () => {
  it('parses the supplied reference TIFF from hex and base64 without throwing', () => {
    const referencePayloads = [
      bytesFromHex(REFERENCE_HEX),
      decodeExifInput(REFERENCE_BASE64, 'base64').bytes,
    ];
    referencePayloads.forEach((bytes) => {
      ...
      expect(ifd0.fields.map((field) => [field.name, field.value]))
        .toContainEqual(['Make', 'ABC']);
      expect(ifd0.fields.map((field) => [field.name, field.value]))
        .toContainEqual(['Model', 'XY']);
      expect(ifd0.fields.map((field) => [field.name, field.value]))
        .toContainEqual(['Orientation', '1']);
  ...
  it('honors big-endian TIFF fields', () => {
    ...
    view.setUint16(18, 8);
    view.setUint32(22, 0);
    expect(parseExif(bytes).groups[0].fields[0])
      .toMatchObject({ name: 'Orientation', value: '8' });
  });

  it('parses JPEG APP1 EXIF and specifically reports a JPEG with no EXIF', () => {
    const tiff = bytesFromHex(REFERENCE_HEX);
    expect(parseExif(makeJpeg(tiff)).groups.some((group) => group.name === 'IFD0'))
      .toBe(true);
    const noExifJpeg = new Uint8Array([0xff, 0xd8, 0xff, 0xd9]).slice();
    expect(parseExif(noExifJpeg).error).toMatch(/No EXIF data found/);
  });

  it('derives signed decimal GPS coordinates and retains raw DMS fields', () => {
    const result = parseExif(buildGpsTiff());
    expect(result.gpsCoordinates).toMatchObject({
      latitude: 37.5,
      longitude: -127.25,
      text: '37.5, -127.25',
    });
    const gpsFields = result.groups.find((group) => group.name === 'GPS IFD').fields;
    expect(gpsFields.map((field) => field.value)).toContain('37/1, 30/1, 0/1');
  });
  ...
  it('reports unknown tags and MakerNote as raw hexadecimal content', () => {
    ...
    const getFirstIfd0Field = () => parseExif(bytes)
      .groups.find((group) => group.name === 'IFD0').fields[0];
    expect(getFirstIfd0Field().name).toBe('Tag 0xC001');
    view.setUint16(10, 0x927c, true);
    expect(getFirstIfd0Field().value).toBe('41424300');
  });
  ...
  it('returns offset-bearing errors for truncation, invalid order, bad offsets, and cycles', () => {
    ...
    const badOffset = reference.slice();
    new DataView(badOffset.buffer).setUint32(54, 9999, true);
    const cycle = reference.slice();
    new DataView(cycle.buffer).setUint32(54, 8, true);
  ...
  it('caps long IFD chains instead of looping forever', () => {
    ...
    Array.from({ length: 36 }, (_, index) => {
      ...
      view.setUint32(offset + 2, index === 35 ? 0 : offset + 6, true);
    });
