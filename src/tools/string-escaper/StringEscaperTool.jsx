...
const LANGUAGE_OPTIONS = [
  [LANGUAGES.JAVASCRIPT, 'JavaScript / JSON'],
  [LANGUAGES.HTML, 'HTML'],
  [LANGUAGES.SQL, 'SQL'],
  [LANGUAGES.JAVA, 'Java / C#'],
...
export default function StringEscaperTool() {
  ...
  return <section className="string-escaper" aria-label="String Escaper Tool">
    ...
    <div className="string-escaper__options" aria-label="Escape options">
      <label>
        Quote style
        <select value={quoteStyle} onChange={(event) => setQuoteStyle(event.target.value)}>
          <option value="double">Double quotes</option>
          <option value="single">Single quotes</option>
        </select>
      </label>
      <label className="string-escaper__checkbox">
        <input
          type="checkbox"
          checked={escapeUnicode}
          onChange={(event) => setEscapeUnicode(event.target.checked)}
        />
        Escape Unicode characters
      </label>
    </div>
    <div className="string-escaper__presets" aria-label="Sample presets">
      <span>Samples:</span>
      {PRESETS.map((preset) => (
        <button
          type="button"
    ...
    <div className="string-escaper__panels">
      <div className="string-escaper__panel">
        <label htmlFor="string-escaper-input">
          {isEscape ? 'Plain text' : 'Escaped text'}
        </label>
        <textarea
          id="string-escaper-input"
          value={input}
          onChange={(event) => setInput(event.target.value)}
          spellCheck={false}
          placeholder={isEscape ? 'Type text to escape…' : 'Paste escaped text to unescape…'}
        />
      </div>
      <div className="string-escaper__panel">
        <div className="string-escaper__output-label">
          <label htmlFor="string-escaper-output">
            {isEscape ? 'Escaped result' : 'Plain text'}
          </label>
          <button type="button" onClick={handleCopy} disabled={!output}>Copy</button>
        </div>
        <textarea
          id="string-escaper-output"
          value={output}
          readOnly
          spellCheck={false}
